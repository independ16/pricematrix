...
## Outstanding Work (as of August 18, 2026)
- [ ] **Next up: WC export/upload for customer specific pricing** (all customers pushed at once, not incremental). Fix `scripts/build-wc-customer-export.js`'s specific-rows-loop mirroring gap first (see scripts/ section above) — without it, Alumatech (601) gets zero pricing in the WC push. Picking this back up in a later session.
- [x] Florida Patio / Alumatech (418) pricing migration — **fully complete**. Replaced ~4,355 stale formula-based `Customer_Pricing_Ratio_STAGING` rows with 344 curated SPECIFIC rows in `Customer_Specific_Pricing` across 6 batches (base set, spray-paint cans, renamed SKUs, restored SKUs, a stale-price correction round, and newly-added-to-WC SKUs), cross-validated against the customer's clean price list and live master catalog after each of two master-data refreshes — zero drift, zero broken references. Of the "orange row" (no-catalog-SKU) items originally held back: `PC-2xx`/`SP-2xx` powder-coat/spray-paint variants priced using David's pack-weight formula (5/10/25/44 lb, and `-C` single-can for spray), `V3-215`/`V3-218` confirmed as colors of already-priced `V3-201` (no separate row needed), 6 SKUs added to WC this session and priced (`FP-105`, `FP-115`, `FS-032`, `FT-157`, `FO-5437`, `FS-043`), and the remaining 8 (`FP-013`, `FP-041`, `FP-096`, `FP-100`, `FP-103`, `FP-104`, `T91N5W118`, `3044376`) confirmed cancelled/discontinued — no further action.
...
- [ ] Patioproducts.com domain in Resend (SPF/DKIM) — eliminates Gmail warning on invite emails. Until this lands, new PriceMatrix users must be created manually (see memory: project-context.md) since Supabase's default mailer isn't reliable for the invite flow.
...
- [x] New user (Karlo Renderos, manager role) created manually via Supabase dashboard while SMTP is blocked — confirmed working login end-to-end
